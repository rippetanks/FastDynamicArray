...
void utest_insert_read_write() {
    ...
    for(i = 0; i < _UNIT_TEST_N; i++)
        TEST_ASSERT_EQUAL_INT32(x, *(int*)dyvec_get(&vector, i));
    for(i = 0; i < _UNIT_TEST_N; i++)
        if(i % 2 == 0)
            dyvec_set(&vector, i, &y);
    for(i = 0; i < _UNIT_TEST_N; i++)
        if(i % 2 == 0)
            TEST_ASSERT_EQUAL_INT32(y, *(int*)dyvec_get(&vector, i));
        else
            TEST_ASSERT_EQUAL_INT32(x, *(int*)dyvec_get(&vector, i));
...
void utest_insert_resize() {
    ...
    dyvec_resize(&vector, _UNIT_TEST_N * 2);
    TEST_ASSERT_EQUAL_INT32(_UNIT_TEST_N, vector.size);
...
void utest_erase_interval() {
    ...
    dyvec_erase_interval(&vector, 0, DYNAMIC_ARRAY_BLOCK_SIZE / 2);
...
void utest_push_pop() {
    ...
    TEST_ASSERT_EQUAL_INT32(x, *(int*)dyvec_pop(&vector));
    for(i = 0; i <= DYNAMIC_ARRAY_BLOCK_SIZE; i++)
        dyvec_push(&vector, &x);
    TEST_ASSERT_EQUAL_INT32(DYNAMIC_ARRAY_BLOCK_SIZE + 1, vector.size);
    dyvec_pop(&vector);
