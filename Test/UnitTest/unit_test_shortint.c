...
void utest_sint_insert_read_write() {
    DynamicVectorShortInt vector;
    int i;
    short int x = 50, y = 100;

    dyvec_sint_init(&vector);

    for(i = 0; i < _UNIT_TEST_N; i++)
        dyvec_sint_add(&vector, x);
    ...
    for(i = 0; i < _UNIT_TEST_N; i++)
        TEST_ASSERT_EQUAL_INT16(x, dyvec_sint_get(&vector, i));
    for(i = 0; i < _UNIT_TEST_N; i++)
        if(i % 2 == 0)
            dyvec_sint_set(&vector, i, y);
    for(i = 0; i < _UNIT_TEST_N; i++)
        if(i % 2 == 0)
            TEST_ASSERT_EQUAL_INT16(y, dyvec_sint_get(&vector, i));
        else
            TEST_ASSERT_EQUAL_INT16(x, dyvec_sint_get(&vector, i));
...
void utest_sint_insert_resize() {
    ...
    dyvec_sint_resize(&vector, _UNIT_TEST_N * 2);
    TEST_ASSERT_EQUAL_INT32(_UNIT_TEST_N, vector.size);
...
void utest_sint_erase_interval() {
    ...
    dyvec_sint_erase_interval(&vector, 0, DYNAMIC_ARRAY_BLOCK_SIZE / 2);
...
void utest_sint_push_pop() {
    ...
    dyvec_sint_init(&vector);
    dyvec_sint_push(&vector, x);
    TEST_ASSERT_EQUAL_INT32(1, vector.size);
    TEST_ASSERT_EQUAL_INT16(x, dyvec_sint_pop(&vector));
    for(i = 0; i <= DYNAMIC_ARRAY_BLOCK_SIZE; i++)
        dyvec_sint_push(&vector, x);
    TEST_ASSERT_EQUAL_INT32(DYNAMIC_ARRAY_BLOCK_SIZE + 1, vector.size);
    dyvec_sint_pop(&vector);
    TEST_ASSERT_EQUAL_INT32(DYNAMIC_ARRAY_BLOCK_SIZE, vector.size);
    for(i = 0; i < DYNAMIC_ARRAY_BLOCK_SIZE - 10; i++)
        dyvec_sint_pop(&vector);
    TEST_ASSERT_EQUAL_INT32(1, vector.block);
